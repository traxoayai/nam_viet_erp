...
export const useBookingResources = () => {
    ...
    const fetchDoctors = useCallback(async () => {
        ...
        try {
            ...
            console.warn('Error fetching doctors (RPC get_users_with_roles might be missing):', err);
     ...
     * Create Customer B2C
     */
    const createCustomer = useCallback(async (customerData: Partial<BookingCustomer>) => {
        setLoading(true);
        try {
     ...
     * Update Customer B2C
     */
    const updateCustomer = useCallback(async (id: number, customerData: Partial<BookingCustomer>) => {
        setLoading(true);
        try {
    ...
    return {
        ...
        actions: useMemo(() => ({
            ...
            createCustomer,
            updateCustomer
        }), [searchCustomers, fetchDoctors, fetchVaccines, createCustomer, updateCustomer])
    };
};
