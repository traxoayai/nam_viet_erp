  ...
  const flattenedData = data.map((item) => {
    ...
    warehouses.forEach((wh) => {
      const settings = item.warehouse_settings?.find(
        (w: unknown) => w.warehouse_id === wh.id
      );
...
export const parseExcelToPayload = (data: unknown[]): unknown[] => {
  return data
    .map((row: unknown) => {
      ...
      if (row["Tên sản phẩm"] !== undefined)
        payload.name = getVal("Tên sản phẩm");
      if (row["Trạng thái"] !== undefined)
        payload.status = getVal("Trạng thái");
      if (row["Ảnh (URL)"] !== undefined)
        payload.image_url = getVal("Ảnh (URL)");
      if (row["Mã vạch"] !== undefined) payload.barcode = getVal("Mã vạch");
      if (row["Nhà sản xuất"] !== undefined)
        payload.manufacturer_name = getVal("Nhà sản xuất");
      if (row["Giá vốn (Cost)"] !== undefined)
        payload.cost_price = getVal("Giá vốn (Cost)", "number");

      // UNIT FIELDS
      if (row["Đơn vị Cơ bản"] !== undefined)
        payload.base_unit_name = getVal("Đơn vị Cơ bản");

      if (row["Đơn vị Lẻ"] !== undefined)
        payload.retail_unit_name = getVal("Đơn vị Lẻ");
      if (row["Quy đổi Lẻ"] !== undefined)
        payload.retail_conversion_rate = getVal("Quy đổi Lẻ", "number");
      if (row["Giá bán lẻ Hiện Tại"] !== undefined)
        payload.retail_price = getVal("Giá bán lẻ Hiện Tại", "number");
      ...
      if (row["Đơn vị Sỉ"] !== undefined)
        payload.wholesale_unit_name = getVal("Đơn vị Sỉ");
      if (row["Quy đổi Sỉ"] !== undefined)
        payload.wholesale_conversion_rate = getVal("Quy đổi Sỉ", "number");
      if (row["Giá bán sỉ Hiện Tại"] !== undefined)
        payload.wholesale_price = getVal("Giá bán sỉ Hiện Tại", "number");
