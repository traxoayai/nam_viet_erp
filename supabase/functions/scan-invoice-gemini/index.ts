...
serve(async (req)=>{
  ...
  try {
    ...
    if (parsedInvoice.invoice_number) {
      ...
      let query = supabase.from("finance_invoices").select("id, status, invoice_number").eq("invoice_number", parsedInvoice.invoice_number);
      ...
      if (!searchError && existingInvoices && existingInvoices.length > 0) {
        ...
        console.log(`[Deduplication] Found existing invoice ID: ${existing.id} Status: ${existing.status}`);
    ...
    if (actionType === "UPDATE") {
      dbResult = await supabase.from("finance_invoices").update(dbPayload).eq("id", targetId).select("id").single();
    } else {
      dbResult = await supabase.from("finance_invoices").insert(dbPayload).select("id").single();
    }
