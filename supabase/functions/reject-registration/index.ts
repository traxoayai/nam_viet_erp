...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { data: request, error: fetchError } = await supabase
      ...
      .select('id, email, business_name, auth_user_id, status')
      .eq('id', request_id)
      .single()
    ...
    if (request.status !== 'pending') {
      return new Response(JSON.stringify({ error: `Yêu cầu đã ở trạng thái "${request.status}", không thể từ chối.` }), {
    ...
    if (updateError) {
      return new Response(JSON.stringify({ error: `Không thể cập nhật trạng thái: ${updateError.message}` }), {
      ...
      await fetch(
        `${Deno.env.get('SUPABASE_URL')}/functions/v1/send-portal-email`,
        {
          ...
          body: JSON.stringify({
            ...
            email: request.email,
            data: { business_name: request.business_name, reason: reason || undefined },
          }),
        },
      )
    } catch (mailErr) {
