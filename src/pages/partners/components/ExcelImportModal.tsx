...
import { Modal, Upload, Button, Table, message, Typography, Steps, Alert } from 'antd';
...
const REQUIRED_HEADERS = [
  'Tên NCC', 'Mã số thuế', 'Địa chỉ', 'Người liên hệ', 'SĐT', 'Email', 
  'Ngân hàng', 'STK', 'Chủ TK', 'Điều khoản TT', 'Cách giao hàng', 
  'Công nợ hiện tại', 'Ghi chú'
];

export const ExcelImportModal: React.FC<ExcelImportModalProps> = ({ open, onCancel, onImport }) => {
  ...
  const handleDownloadTemplate = () => {
    const ws = XLSX.utils.aoa_to_sheet([
      REQUIRED_HEADERS,
      ['Công ty Senko', '0101234567', 'TP.HCM', 'Anh Nam', '0909123456', 'nam@senko.vn', 'VCB', '123456', 'NGUYEN VAN A', '30 ngày', 'Giao tận nơi', 5000000, 'Ghi chú mẫu']
    ]);
  ...
  const handleFile = (file: File) => {
    ...
    reader.onload = (e) => {
      try {
        ...
        if (headers[0] !== REQUIRED_HEADERS[0] || headers[4] !== REQUIRED_HEADERS[4]) {
             message.warning("Cấu trúc file có vẻ không đúng mẫu. Vui lòng tải mẫu mới nhất.");
        }
  ...
  const columns = [
      ...
      { title: 'Công nợ', dataIndex: 'current_debt', key: 'debt', render: (val: number) => val?.toLocaleString() },
  ...
  return (
    ...
    >
        <div style={{ marginBottom: 16 }}>
            <Steps current={step} items={[{ title: 'Tải lên' }, { title: 'Kiểm tra & Xác nhận' }]} />
        </div>
        ...
        {step === 0 && (
            <div style={{ textAlign: 'center', padding: 20 }}>
                <Alert 
                  ...
                  description={
                    <span>
                      Vui lòng sử dụng <a onClick={handleDownloadTemplate}>File Mẫu chuẩn</a> để nhập liệu.
                      <br/>
        ...
        {step === 1 && (
            <div>
                 ...
                 <div style={{ marginTop: 16, textAlign: 'right' }}>
                    <Text type="secondary">Tổng số dòng hợp lệ: {validData.length}</Text>
                 </div>
                 <div style={{ marginTop: 16 }}>
                     <Button type="dashed" icon={<UploadOutlined />} onClick={() => setStep(0)} disabled={uploading}>
                         Chọn file khác
                     </Button>
                 </div>
            </div>
