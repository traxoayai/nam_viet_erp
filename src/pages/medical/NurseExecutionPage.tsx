...
const ObservingCountdown = ({ startTime }: { startTime?: string }) => {
  ...
  const obsTime = startTime
    ? dayjs(startTime).add(30, "minute").valueOf()
    : dayjs().add(30, "minute").valueOf();
  const [complete, setComplete] = useState(Date.now() >= obsTime);

  return (
    <div
      className={`mt-2 p-2 rounded text-center font-bold ${complete ? "bg-green-100 text-green-700 animate-pulse" : "bg-blue-50 text-blue-600"}`}
    >
      {complete ? (
        <span className="flex items-center justify-center gap-1">
          <CheckCircle size={16} /> ĐÃ ĐẾN GIỜ RA VỀ
        </span>
      ) : (
        <Statistic.Countdown
          value={obsTime}
          ...
          valueStyle={{ fontSize: 14, color: "#2563eb", fontWeight: "bold" }}
        />
      )}
    </div>
  );
...
export default function NurseExecutionPage() {
  ...
  const displayedQueue = queue.filter((item) => {
    ...
    if (
      serviceTypeFilter === "vaccination" &&
      item.service_type !== "vaccination"
    )
      return false;
    if (
      serviceTypeFilter === "examination" &&
      item.service_type === "vaccination"
    )
      return false; // Thử thuật thường là mảng khác vaccination

    return true;
  });

  const selectedPatient = queue.find(
    (q) => q.customer_id === selectedPatientId
  );
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-gray-500 mt-1">
                {filterDate.format("dddd, DD/MM/YYYY")} • Cần thực hiện:{" "}
                {queue.filter((q) => q.status !== "observing").length} | Đang
                theo dõi: {queue.filter((q) => q.status === "observing").length}
              </p>
            </div>
          </div>
      ...
      <div className="flex flex-1 overflow-hidden bg-gray-50">
        ...
        <div className="w-[30%] bg-white border-r border-gray-200 flex flex-col">
          ...
          <Tabs
            ...
            items={[
              {
                label: <span className="font-bold">Chờ Thực Hiện</span>,
                key: "waiting",
              },
              {
                label: <span className="font-bold">Theo Dõi 30P</span>,
                key: "observing",
              },
            ]}
          />
                ...
                return (
                  ...
                  >
                    <div className="flex justify-between items-start mb-2">
                      <div className="flex items-center gap-2">
                        <div
                          className={`w-8 h-8 rounded shrink-0 flex items-center justify-center font-bold text-white ${isVaccination ? "bg-purple-500" : "bg-blue-500"}`}
                        >
        ...
        <div className="w-[70%] bg-slate-50 p-6 flex flex-col h-full overflow-y-auto">
          {!selectedPatient ? (
            ...
            <div className="flex flex-col h-full animate-in fade-in zoom-in-95 duration-200">
              ...
              {selectedPatient.red_flags &&
              selectedPatient.red_flags.length > 0 ? (
                <div className="bg-red-50 border-2 border-red-500 rounded-xl p-4 mb-6 shadow-sm shadow-red-200 animate-pulse ring-4 ring-red-500/30">
                  ...
                  <div className="pl-9 font-bold text-red-600 text-base">
                    {selectedPatient.red_flags.map(
                      (rf: string, idx: number) => (
                        <div key={idx}>• {rf}</div>
                      )
                    )}
                  </div>
              ...
              <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-5 mb-6 flex items-start gap-4">
                ...
                <div className="flex-1">
                  ...
                  <div className="flex gap-4 text-sm text-gray-600">
                    <span>
                      Giới tính:{" "}
                      <b>{selectedPatient.gender === "male" ? "Nam" : "Nữ"}</b>
                    </span>
                    <span>
                      Tuổi: <b>{dayjs().year() - selectedPatient.yob}</b>
                    </span>
                    <span>
                      SĐT: <b>{selectedPatient.customer_phone}</b>
                    </span>
                  </div>
                </div>
                <div className="text-right">
                  ...
                  {selectedPatient.service_type === "vaccination" ? (
                    <Tag
                      color="purple"
                      className="m-0 text-sm py-1 px-3 font-bold border-purple-300"
                    >
                      💉 TIÊM CHỦNG
                    </Tag>
                  ) : (
                    <Tag
                      color="blue"
                      className="m-0 text-sm py-1 px-3 font-bold border-blue-300"
                    >
                      ✂️ THỦ THUẬT
                    </Tag>
                  )}
              ...
              {selectedPatient.status === "observing" ? (
                <div className="bg-green-50 rounded-xl border border-green-200 p-10 flex flex-col items-center justify-center h-64 text-center">
                  ...
                  <h3 className="text-2xl font-bold text-green-800">
                    Đã Ký Bằng Thao Tác - Đang Theo Dõi 30P
                  </h3>
                  <p className="text-green-600 mt-2 text-lg">
                    Vui lòng hướng dẫn bệnh nhân ngồi tại khu vực chờ quy định.
                  </p>
                </div>
              ) : selectedPatient.service_type === "vaccination" ? (
                <VaccineWorkstation
                  ...
                  onConfirm={(scannedIds) =>
                    handleConfirmVaccination(scannedIds, selectedPatient)
                  }
                />
              ) : (
                <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6 flex flex-col h-full">
                  <h3 className="text-lg font-bold text-gray-800 mb-4 border-b pb-2">
                    DANH SÁCH THỦ THUẬT CẦN THỰC HIỆN
                  </h3>

                  {selectedPatient.procedures?.length > 0 ? (
                    <div className="flex flex-col gap-3">
                      {selectedPatient.procedures.map(
                        (proc: unknown, idx: number) => (
                          <div
                            key={idx}
