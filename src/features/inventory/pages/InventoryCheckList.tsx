...
export const InventoryCheckList = () => {
    ...
    }, [filters.warehouseId, filters.status, filters.dateRange, pagination.current, pagination.pageSize]); 
    ...
    const columns = [
        ...
        },
        {
            ...
            key: 'created_at',
            render: (d: string) => <span style={{fontSize: 13}}>{dayjs(d).format('DD/MM/YYYY HH:mm')}</span>,
        },
        {
            title: 'Người tạo',
        ...
        },
        {
            ...
            render: (status: string) => {
                let color = 'default';
                let text = status;
                if (status === 'DRAFT') { color = 'processing'; text = 'Đang kiểm'; }
        ...
        },
        {
            ...
            render: (val: number) => (
                <Text type={val < 0 ? 'danger' : (val > 0 ? 'success' : 'secondary')} strong>
    ...
    return (
        <Layout style={{ padding: '24px', background: '#f0f2f5', minHeight: '100vh' }}>
            ...
            <div style={{ marginBottom: 20, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <div>
                    <Title level={3} style={{ margin: 0 }}>Lịch Sử Kiểm Kê</Title>
                    <Text type="secondary">Theo dõi các đợt kiểm kho và xử lý chênh lệch</Text>
                </div>
            ...
            <Card styles={{ body: { padding: 16 } }} style={{marginBottom: 16, borderRadius: 8}}>
                <Row gutter={[16, 16]} align="middle">
                    ...
                    <Col xs={24} sm={12} md={6} lg={5}>
                        <div style={{fontSize: 12, color: '#888', marginBottom: 4}}>Kho kiểm kê:</div>
                        <Select 
                            style={{width: '100%'}}
                    ...
                    <Col xs={24} sm={12} md={6} lg={6}>
                        <div style={{fontSize: 12, color: '#888', marginBottom: 4}}>Tìm kiếm:</div>
                        <Input 
                    ...
                    <Col xs={12} sm={16} md={6} lg={6}>
                        <div style={{fontSize: 12, color: '#888', marginBottom: 4}}>Thời gian:</div>
                        <RangePicker 
                            style={{width: '100%'}} 
                            format="DD/MM/YYYY"
                            onChange={(dates) => {
                    ...
                    <Col xs={24} sm={24} md={2} lg={3} style={{display:'flex', alignItems:'end'}}>
                         <Button type="primary" ghost icon={<SearchOutlined />} onClick={handleSearch} block>
                            Lọc
                        </Button>
                    </Col>
                </Row>
            ...
            <Content>
                <Table 
                    ...
                    style={{ background: '#fff', borderRadius: 8, padding: 0, overflow:'hidden' }}
                />
            </Content>
