...
describe("Phân quyền write kế toán (finance.post/void/close_journal)", () => {
  ...
  it("role Kế Toán có BỘ quyền nhất quán: view_balance + post/void/close (vào được trang + duyệt)", async () => {
    const { data, error } = await adminClient
      .from("role_permissions")
      .select("permission_key, roles!inner(name)")
      .eq("roles.name", "Kế Toán");
    expect(error).toBeNull();
    const keys = (data as { permission_key: string }[]).map(
      (r) => r.permission_key
    );
    // view_balance để qua route-gate trang Sổ/BCTC; 3 quyền write để duyệt/hủy/khóa kỳ
    expect(keys).toContain("finance.view_balance");
    expect(keys).toContain("finance.post_journal");
    expect(keys).toContain("finance.void_journal");
    expect(keys).toContain("finance.close_period");
  }, 30000);
});
...
describe("gen_journal_for_sales_order — ghi sổ doanh thu + giá vốn từ đơn", () => {
  ...
  beforeAll(async () => {
    ...
    await pg.connect();
    try {
      // Đơn bán thật: final_amount = REVENUE, status DELIVERED (qua status-gate)
      const { rows } = await pg.query(
        `INSERT INTO orders(code, order_type, status, final_amount, total_amount, created_at)
         VALUES('TEST-ACC-ORD-P4','POS','DELIVERED',$1,$1,'2028-05-10') RETURNING id`,
        [REVENUE]
      );
  ...
  // ── v2 fixes (adversarial review) ─────────────────────────────────────────
  it("status-gate: đơn PENDING (chưa xuất kho) bị bỏ qua (status_not_recognizable)", async () => {
    const client = await createTestAuthedClient();
    const pg = new Client(PG_CFG);
    await pg.connect();
    let oid: string;
    try {
      const { rows } = await pg.query(
        `INSERT INTO orders(code, order_type, status, final_amount, total_amount, created_at)
         VALUES('TEST-ACC-PENDING','B2B','PENDING',100000,100000,'2028-05-11') RETURNING id`
      );
      oid = rows[0].id as string;
    } finally {
      await pg.end();
    }
    try {
      ...
      expect(error).toBeNull();
      expect((data as { skipped?: string }).skipped).toBe(
        "status_not_recognizable"
      );
    } finally {
  ...
  it("per-doc_type idempotency: xóa bút toán cogs rồi gọi lại → tạo LẠI cogs (không bị chặn)", async () => {
    const client = await createTestAuthedClient();
    const pg = new Client(PG_CFG);
    await pg.connect();
    let oid: string;
    try {
      const { rows } = await pg.query(
        `INSERT INTO orders(code, order_type, status, final_amount, total_amount, created_at)
         VALUES('TEST-ACC-PARTIAL','POS','DELIVERED',400000,400000,'2028-05-12') RETURNING id`
      );
      oid = rows[0].id as string;
      await pg.query(
        `INSERT INTO order_items(order_id, product_id, quantity, uom, unit_price, is_gift)
         VALUES($1,$2,1,'Hộp',400000,false)`,
        [oid, PROD_A]
      );
    } finally {
      await pg.end();
    ...
    try {
      // Lần 1: tạo cả sale + cogs
      const r1 = await client.rpc("gen_journal_for_sales_order", {
        p_order_id: oid,
      });
      expect(r1.error).toBeNull();
      expect(
        (r1.data as { entry_cogs: number | null }).entry_cogs
      ).not.toBeNull();

      // Mô phỏng trạng thái thiếu cogs (vd actual_cost=0 lúc đầu): xóa bút toán cogs
      const pgx = new Client(PG_CFG);
      await pgx.connect();
      try {
        await pgx.query(
          `DELETE FROM journal_entry_lines WHERE entry_id IN (
             SELECT id FROM journal_entries WHERE source_ref_type='orders' AND source_ref_id=$1 AND doc_type='cogs')`,
          [oid]
        );
        await pgx.query(
          `DELETE FROM journal_entries WHERE source_ref_type='orders' AND source_ref_id=$1 AND doc_type='cogs'`,
          [oid]
        );
      } finally {
        await pgx.end();
      ...
      // Lần 2: phải TẠO LẠI cogs (per-doc_type), KHÔNG trả 'already_booked'
      const r2 = await client.rpc("gen_journal_for_sales_order", {
        p_order_id: oid,
      });
      expect(r2.error).toBeNull();
      expect(
        (r2.data as { entry_cogs: number | null }).entry_cogs
      ).not.toBeNull();

      // DB: đúng 1 sale + 1 cogs (không nhân đôi sale)
      const pgz = new Client(PG_CFG);
      await pgz.connect();
      try {
        const { rows } = await pgz.query(
          `SELECT doc_type, count(*)::int n FROM journal_entries
           WHERE source_ref_type='orders' AND source_ref_id=$1 GROUP BY doc_type`,
          [oid]
        );
        const m = Object.fromEntries(rows.map((r) => [r.doc_type, r.n]));
        expect(m["sale"]).toBe(1);
        expect(m["cogs"]).toBe(1);
      } finally {
        await pgz.end();
      }
    } finally {
  ...
  it("tách phí ship: 131=final, 5111=final-ship, 5113=ship", async () => {
    const client = await createTestAuthedClient();
    const pg = new Client(PG_CFG);
    await pg.connect();
    let oid: string;
    try {
      const { rows } = await pg.query(
        `INSERT INTO orders(code, order_type, status, final_amount, total_amount, shipping_fee, created_at)
         VALUES('TEST-ACC-SHIP','B2B','PACKED',520000,500000,20000,'2028-05-13') RETURNING id`
      );
      oid = rows[0].id as string;
      await pg.query(
        `INSERT INTO order_items(order_id, product_id, quantity, uom, unit_price, is_gift)
         VALUES($1,$2,1,'Hộp',500000,false)`,
        [oid, PROD_A]
      );
    } finally {
      await pg.end();
    ...
    try {
      ...
      expect(error).toBeNull();
      const res = data as { revenue: number; shipping: number };
      expect(Number(res.revenue)).toBe(500000);
      expect(Number(res.shipping)).toBe(20000);

      const pg2 = new Client(PG_CFG);
      await pg2.connect();
      try {
        const { rows } = await pg2.query(
          `SELECT a.account_code, l.debit, l.credit FROM journal_entry_lines l
           JOIN journal_entries e ON e.id=l.entry_id JOIN chart_of_accounts a ON a.id=l.account_id
           WHERE e.source_ref_type='orders' AND e.source_ref_id=$1 AND e.doc_type='sale' ORDER BY l.line_no`,
          [oid]
        );
        const byCode = Object.fromEntries(rows.map((r) => [r.account_code, r]));
        expect(Number(byCode["131"].debit)).toBe(520000);
        expect(Number(byCode["5111"].credit)).toBe(500000);
        expect(Number(byCode["5113"].credit)).toBe(20000);
      } finally {
        await pg2.end();
      }
    } finally {
