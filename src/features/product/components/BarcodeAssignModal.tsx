...
export const BarcodeAssignModal: React.FC<Props> = ({ visible, scannedBarcode, onCancel, onSuccess }) => {
    ...
    const handleProductSelect = (productId: number) => {
        ...
        if (prod) {
            ...
            const unitOptions = prod.product_units?.map((u:any) => ({
                id: u.id,
                name: u.unit_name
            })) || [];
    ...
    return (
        <Modal
            title={<span><BarcodeOutlined /> Gán mã vạch mới</span>}
            open={visible}
            onCancel={onCancel}
            onOk={handleOk}
        ...
        >
            <div className="mb-4 text-center">
                ...
                <div className="text-2xl font-bold text-blue-600 my-1">{scannedBarcode}</div>
                <Tag color="red">Chưa tồn tại trong hệ thống</Tag>
            </div>

            <Form form={form} layout="vertical">
                <Form.Item name="product_id" label="Tìm và chọn sản phẩm để gán:" rules={[{ required: true, message: 'Vui lòng chọn sản phẩm' }]}>
                    <Select
                        showSearch
                        ...
                        loading={loading}
                        notFoundContent={loading ? <div className="p-2 text-center text-gray-400">Đang tìm...</div> : null}
                    >
                        {products.map(p => (
                ...
                <Form.Item name="unit_id" label="Gán cho đơn vị tính nào?" rules={[{ required: true, message: 'Vui lòng chọn đơn vị' }]}>
                    <Select placeholder="Chọn đơn vị (Hộp/Viên/Vỉ...)">
                        {/* [FIX] Dùng ID làm Key và Value -> Hết lỗi Duplicate Key */}
                        {units.map(u => (
                            <Select.Option key={u.id} value={u.id}>
                                {u.name} <span className="text-gray-400 text-xs ms-1">#{u.id}</span>
                            </Select.Option>
                        ))}
                    </Select>
                </Form.Item>
