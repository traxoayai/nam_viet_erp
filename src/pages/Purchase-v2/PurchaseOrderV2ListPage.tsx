...
export default function PurchaseOrderV2ListPage() {
  ...
  const [financeInitialValues, setFinanceInitialValues] =
  ...
  const {
    ...
    isLoading,
    deleteOrder,
  } = usePurchaseOrdersV2(searchTerm);
  ...
  const handlePaySupplier = async (record: unknown) => {
    try {
      message.loading({
        content: "Đang tải thông tin NCC...",
        key: "pay_supplier",
      });
      ...
      message.error({
        content: "Lỗi lấy thông tin NCC: " + err.message,
        key: "pay_supplier",
      });
    }
  };

  const handlePayShipping = async (record: unknown) => {
    ...
    try {
      message.loading({
        content: "Đang tải thông tin ĐVVC...",
        key: "pay_shipping",
      });
      ...
      message.error({
        content: "Lỗi lấy thông tin ĐVVC: " + err.message,
        key: "pay_shipping",
      });
    }
  };
  ...
  const columns = [
    {
      ...
      key: "code",
      width: 120,
      sorter: (a: unknown, b: unknown) => a.code.localeCompare(b.code),
    },
    {
      title: "Ngày tạo",
    ...
    },
    {
      ...
      render: (_: unknown, record: unknown) => (
        <strong>{record.suppliers?.name || "-"}</strong>
      ),
    },
    {
      title: "Vận Chuyển",
        ...
        return (
          <div>
            ...
            <div style={{ fontSize: "0.85em", color: "#666", marginTop: 4 }}>
              Đã trả:{" "}
              {new Intl.NumberFormat("vi-VN").format(record.total_paid || 0)} đ
            </div>
          </div>
        );
      },
    },
    {
      ...
      render: (_: unknown, record: unknown) => (
        <Space size="middle">
          <Popconfirm
            title={
              isAdmin ? "Xóa CỨNG đơn hàng này?" : "Hủy (xóa mềm) đơn hàng này?"
            }
            onConfirm={() =>
              deleteOrder({ id: record.id, hardDelete: isAdmin })
            }
          >
          ...
          >
            ...
            <Space>
              ...
              <Button
                ...
                onClick={() =>
                  message.info(
                    "Chức năng thanh toán hàng loạt đang hoàn thiện."
                  )
                }
              >
                Thanh toán
              ...
              <Popconfirm
                title={
                  isAdmin
                    ? "Xóa CỨNG các đơn đã chọn?"
                    : "Hủy (xóa mềm) các đơn đã chọn?"
                }
