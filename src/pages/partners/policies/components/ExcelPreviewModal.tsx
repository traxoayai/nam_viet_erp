...
import { DeleteOutlined, CheckCircleOutlined, WarningOutlined } from '@ant-design/icons';
...
export const ExcelPreviewModal: React.FC<Props> = ({ open, data, onCancel, onConfirm }) => {
    ...
    useEffect(() => {
        if (open && data) {
            ...
            const mapped = data.map((d, index) => ({
                ...
                excel_name: d.excel_name || d.raw_name || d.input_name || d.name || "N/A", // Fallback keys
                excel_sku:d.excel_sku || d.raw_sku || d.input_sku || d.sku || "",
                
                system_product_id: d.similarity_score >= 0.8 ? d.product_id : undefined,
                system_product_name: d.similarity_score >= 0.8 ? d.product_name : undefined,
                system_product_sku: d.similarity_score >= 0.8 ? d.product_sku : undefined,
    ...
    const handleConfirm = () => {
        const validItems = items.filter(i => i.system_product_id).map(i => ({
...
// 1. Hàm fetch dữ liệu đúng chuẩn RPC B2B
    const fetchProductOptions = async (search: string) => {
        const { data, error } = await supabase.rpc('search_products_for_b2b_order', {
            p_keyword: search || '',
            p_warehouse_id: 1 
        });
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: 4 }}>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column' }}>
                        ...
                        <span style={{ fontSize: 11, color: '#666' }}>
                            {p.sku} | ĐV Sỉ: <span style={{ color: '#1890ff' }}>{p.wholesale_unit || 'N/A'}</span>
                        </span>
                    </div>
                </div>
            ),
    ...
    const columns = [
        {
            ...
            render: (_: any, r: ExcelItem) => (
                <div>
                    ...
                    {r.excel_sku && <Tag>{r.excel_sku}</Tag>}
                </div>
            )
        },
        {
            ...
            render: (_: any, r: ExcelItem) => (
                <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                    ...
                    {r.match_score !== undefined && r.match_score >= 0.8 && (
                       <div style={{ fontSize: 11, color: '#52c41a' }}><CheckCircleOutlined /> Tự động khớp cao</div>
                    )}
                    {(!r.system_product_id && (r.match_score || 0) < 0.8) && (
                       <div style={{ fontSize: 11, color: '#faad14' }}><WarningOutlined /> Chưa khớp</div>
                    )}
                </div>
            )
        },
            ...
            footer={[
                <div key="footer" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                     ...
                     <Space>
                         ...
                         <Button type="primary" onClick={handleConfirm} disabled={stats.matched === 0}>
