...
function parseArgs() {
  ...
  if (!clientId || !clientSecret) {
    ...
    console.error(
      "Usage: node scripts/get-gmail-refresh-token.js <CLIENT_ID> <CLIENT_SECRET>"
    );
    console.error(
      `Hoac tao file ${ENV_FILE} voi GMAIL_CLIENT_ID + GMAIL_CLIENT_SECRET`
    );
    process.exit(1);
  }
...
async function exchangeCodeForTokens(code, clientId, clientSecret) {
  ...
  if (!res.ok)
    throw new Error(`OAuth exchange failed: ${JSON.stringify(json)}`);
  return json;
}
...
function main() {
  ...
  const server = http.createServer(async (req, res) => {
    ...
    try {
      ...
      if (!tokens.refresh_token) {
        ...
        console.warn(
          "   Vao https://myaccount.google.com/permissions, remove app nay,"
        );
  ...
  server.listen(PORT, () => {
    console.log(
      `⏳ Dang cho OAuth callback tai http://localhost:${PORT}/oauth2callback...`
    );
  });
}
