...
import { Modal, Table, InputNumber, Button, Typography, Space, Descriptions, Tooltip, message, Row, Col } from 'antd';
...
export const CostAllocationModal: React.FC<CostAllocationModalProps> = ({ 
    ...
    const recalcCosts = (currentItems: any[]) => {
        const calculated = currentItems.map(item => {
            ...
            const totalCostLanded = (rawTotal - rebateVal + vatVal) + (item.allocated_shipping_fee || 0) + (otherFee / currentItems.length); // Simple split for other fee? Or just ignore other fee in cost per unit? Req says Ship allocation. Let's keep other fee separate or ask. 
            ...
            const finalUnitCost = totalQty > 0 
                ? Math.round(totalCostLanded / totalQty)
                : 0;
    ...
    // Summary Stats
    const totalImportValue = dataSource.reduce((sum, item) => sum + (item.final_unit_cost * (item.quantity + item.bonus_quantity)), 0);
    const totalRebate = dataSource.reduce((sum, item) => sum + (item.unit_price * item.quantity * (item.rebate_rate / 100)), 0);
    ...
    const columns: any[] = [
        {
            ...
            render: (text: string, r: any) => (
                <div>
                    ...
                    <div><Text type="secondary">{r.sku}</Text></div>
                </div>
            )
        },
        {
        ...
        },
        {
            ...
            render: (v: number, r: any) => (
                <InputNumber 
                    min={0} max={100} 
                    value={v} 
                    onChange={val => handleItemChange(r.product_id, 'vat_rate', val || 0)} 
                    size="small"
                />
            )
        },
        {
            ...
            render: (v: number, r: any) => (
                <InputNumber 
                    min={0} max={100} 
                    value={v} 
                    onChange={val => handleItemChange(r.product_id, 'rebate_rate', val || 0)}
                    size="small"
        ...
        },
        {
            ...
            render: (v: number, r: any) => (
                <InputNumber 
                    min={0} 
                    value={v} 
                    onChange={val => handleItemChange(r.product_id, 'bonus_quantity', val || 0)}
                    size="small"
                />
            )
        },
        {
            ...
            render: (v: number, r: any) => (
                <InputNumber 
                    min={0}
                    value={v} 
                    onChange={val => handleItemChange(r.product_id, 'allocated_shipping_fee', val || 0)}
                    size="small"
                    parser={(displayVal) => displayVal ? Math.round(Number(displayVal)) : 0}
                />
            )
        },
        {
            ...
            fixed: 'right',
            align: 'right',
            render: (v: number) => <Text strong style={{ color: '#096dd9' }}>{v?.toLocaleString()}</Text>
        }
    ];

    return (
        <Modal
            title={<Space><CalculatorOutlined /> Tính Giá Vốn Nhập Hàng (Landed Cost)</Space>}
            open={open}
            onCancel={onCancel}
            width={1200}
            footer={[
        ...
        >
                    ...
                    <Col span={8}>
                       <Descriptions size="small" column={1}>
                            <Descriptions.Item label="Tổng tiền hàng (Base)">
                                <b>{items.reduce((s, i) => s + (i.unit_price * i.quantity), 0).toLocaleString()}</b>
                            </Descriptions.Item>
                       </Descriptions>
                    </Col>
                    <Col span={16}>
                        <Space size="large">
                            <div>
                                <Text>Phí Vận Chuyển (Tổng):</Text><br/>
                                <InputNumber 
                                    style={{ width: 150 }} 
                                    value={totalShipping} 
                            ...
                            <Button type="dashed" onClick={handleAllocate} icon={<CalculatorOutlined />}>
                                Phân bổ tự động
                            </Button>
                        </Space>
                    </Col>
                     ...
                     <Table.Summary fixed>
                        <Table.Summary.Row>
                            ...
                            <Table.Summary.Cell index={1} align="right">
                                <Text type="danger" strong>{totalImportValue.toLocaleString()}đ</Text>
                            </Table.Summary.Cell>
                        </Table.Summary.Row>
                     </Table.Summary>
                )}
            ...
            <div style={{ marginTop: 16, textAlign: 'right' }}>
                 <Space direction="vertical" align="end">
                    <Text type="success">Tổng Rebate tích lũy ví NCC: +{totalRebate.toLocaleString()}đ</Text>
                     <Text type="secondary" style={{ fontSize: 13, fontStyle: 'italic', color: '#faad14' }}>
                        * Lưu ý: Số tiền chiết khấu (Rebate) sẽ được cộng vào Ví NCC. Giá trị thanh toán của đơn hàng vẫn giữ nguyên.
                    </Text>
