...
export const useBulkPaymentAllocation = (customerId?: number) => {
    ...
    const fetchOrders = async () => {
      ...
      try {
        ...
        const orders = data.map((o: unknown) => {
          ...
          return {
            ...order,
            need_to_collect: Number(order.final_amount) - Number(order.paid_amount || 0),
          };
        });
  ...
  const totalAllocated = useMemo(() => {
    return allocatedOrders.reduce(
      (sum, o) => moneyAdd(sum, o.allocated_amount),
      0
    );
  }, [allocatedOrders]);

  const remainingToAdvance =
    totalReceived > totalAllocated
      ? moneySub(totalReceived, totalAllocated)
      : 0;

  // Hàm tự động check theo số tiền (Auto-tick)
  const handleTotalReceivedChange = useCallback(
    (amount: number) => {
