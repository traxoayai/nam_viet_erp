...
test.describe("Auth Flow", () => {
  ...
  test("login fail -> stays on login or shows error", async ({ page }) => {
    ...
    const emailInput = page
      .locator("#email, input[id*='email'], input[type='email']")
      .first();
    ...
    const hasError =
      (await page.locator(".ant-message-error, .ant-alert-error").count()) > 0;
    expect(stillOnAuth || hasError).toBeTruthy();
  });

  test("unauthenticated -> blocked by permission gate or login", async ({
    page,
  }) => {
    await page.goto("/purchase-orders");
    await page.waitForTimeout(5000);
    const gateVisible =
      (await page.locator("text=Yêu cầu Truy cập").count()) > 0;
