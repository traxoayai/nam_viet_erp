...
async function sendPortalEmail(params) {
  ...
  const response = await fetch(
    `${supabaseUrl}/functions/v1/send-portal-email`,
    {
      method: "POST",
      headers: {
  ...
  if (!response.ok) {
    ...
    const message =
      body?.error || `send-portal-email failed with status ${response.status}`;
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const portalUrl =
      Deno.env.get("PORTAL_SITE_URL") ?? "https://nam-viet-b2b.vercel.app";
    ...
    if (existing) {
      ...
      await sendPortalEmail({
        ...
        displayName:
          display_name || existing.user_metadata?.display_name || email,
      });
      return new Response(
        JSON.stringify({
