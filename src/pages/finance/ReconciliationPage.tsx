...
const ReconciliationPage = () => {
  ...
  const handleUpload = async (file: File) => {
    ...
    try {
      ...
      const results: ReconciliationMatch[] = transactions.map(
        (trans, index) => {
          ...
          if (codeMatch) {
            ...
            matchedOrder = pendingOrders.find((o) =>
              o.order_code.includes(extractedCode)
            );
          }
          ...
          if (!matchedOrder) {
            matchedOrder = pendingOrders.find((o) => {
              ...
              const isAmountMatch =
                Math.abs(trans.amount - o.remaining_amount) < 1000;
          ...
          return {
            ...
            status: (matchedOrder ? "matched" : "unmatched") as
              | "matched"
              | "unmatched",
          };
        }
      ...
      setMatches(results);
      if (results.length > 0)
        message.success(`Đã đọc được ${results.length} giao dịch từ sao kê.`);
    } catch (err: unknown) {
  ...
  const handleConfirmReconciliation = async () => {
    ...
    if (orderIds.length === 0)
      return message.warning("Chưa ghép đôi đơn hàng nào.");
  ...
  const matchColumns = [
    {
      ...
      width: "45%",
      render: (trans: unknown) => (
        <div>
          <div style={{ fontWeight: 500, color: "#1890ff" }}>
            {trans.date || "N/A"} -{" "}
            {new Intl.NumberFormat("vi-VN").format(trans.amount)}đ
          </div>
    ...
    },
    {
      ...
      width: "45%",
      render: (val: string, record: unknown) => (
        <Select
          showSearch
          style={{ width: "100%" }}
