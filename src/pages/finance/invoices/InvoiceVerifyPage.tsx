...
const InvoiceVerifyPage = () => {
    ...
    const [selectedProductsMap, setSelectedProductsMap] = useState<Record<number, any>>({});
    ...
    const handleSelectProduct = async (product: any) => {
         ...
         // Update Cache
         setSelectedProductsMap(prev => ({ ...prev, [fullProduct.id]: fullProduct }));
  ...
  const columns = [
      ...
      }] : []),
    {
      ...
      render: (_: any, _record: any, index: number) => (
        <Form.Item
          shouldUpdate={(prev, curr) => prev.items?.[index]?.product_id !== curr.items?.[index]?.product_id}
        ...
        >
          {({ getFieldValue }) => {
              ...
              const selectedProduct = selectedProductsMap[productId] || products.find(p => p.id === productId);
                  ...
                  <div style={{ width: '100%' }}>
                      ...
                      {!selectedProduct ? (
                          ...
                          >
                             {/* Close Button */}
                             <div 
                                style={{ position: 'absolute', top: -8, right: -8, zIndex: 10, background: '#fff', borderRadius: '50%' }}
                                onClick={(e) => {
                                    e.stopPropagation();
                             ...
                             <div style={{ fontWeight: 'bold', color: '#096dd9', fontSize: 13 }}>
                                 {selectedProduct.sku}
                             </div>
                             <div style={{ fontSize: 13, color: '#262626', lineHeight: 1.2 }}>
    ...
    },
    {
      ...
      render: (_: any, _record: any, index: number) => (
         <Form.Item shouldUpdate={(prev, curr) => 
            ...
            prev.items?.[index]?.internal_unit !== curr.items?.[index]?.internal_unit
         }>
            {({ getFieldValue }) => {
                const productId = getFieldValue(["items", index, "product_id"]);
                // Cache lookup
                ...
                if (!selectedProduct) return <span style={{color: '#ccc', fontStyle: 'italic'}}>Chưa khớp SP</span>;

                const currentQty = getFieldValue(["items", index, "quantity"]) || 0;
                const currentUnitId = getFieldValue(["items", index, "internal_unit"]);
                const baseXmlQty = getFieldValue(["items", index, "xml_quantity"]) || 0;
                
                const units = selectedProduct.product_units || selectedProduct.units || [];
                const selectedUnitObj = units.find((u: any) => u.id === currentUnitId);
                ...
                return (
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                        ...
                        >
                            ...
                            >
                                {units.map((u: any) => (
                                    <Select.Option key={u.id} value={u.id} data_rate={u.conversion_rate}>
  ...
  return (
    <Layout style={{ minHeight: "100vh" }}>
      <div style={{ padding: "12px 24px", background: "#fff", borderBottom: "1px solid #f0f0f0", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        ...
        {!isReadOnly && (
          <Space>
             <Button icon={<SaveOutlined />} loading={loading} onClick={() => onSaveDraft(form.getFieldsValue())}>
                Lưu Nháp
             </Button>
             <Button type="primary" icon={<CheckCircleOutlined />} loading={loading} onClick={() => form.submit()}>
                Lưu & Nhập Kho
             </Button>
          </Space>
        )}
