...
export const usePickingListPrint = () => {
    ...
    const printById = async (orderId: string | number) => {
        try {
            ...
            const mappedInfo: OutboundOrderInfo = {
                ...
                delivery_address: detail.delivery_address || detail.customer?.shipping_address || "",
                note: detail.note || "",
            ...
            const mappedItems: OutboundPickItem[] = rawItems.map((i: any) => {
                ...
                if (Array.isArray(invList) && invList.length > 0) {
                    ...
                    const hasStockInv = invList.find((inv: any) => inv.stock_quantity > 0);
    ...
    const printByData = (orderInfo: Partial<OutboundOrderInfo>, items: OutboundPickItem[]) => {
        setIsPrinting(true);
