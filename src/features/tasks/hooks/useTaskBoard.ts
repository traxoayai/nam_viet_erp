...
export const useTaskBoard = () => {
  ...
  const updateStatusMutation = useMutation({
    ...
    onError: (err, _variables, context) => {
      ...
      message.error(
        "Cập nhật trạng thái thất bại, hệ thống tự động Rollback! Lỗi: " +
          err.message
      );
    },
    onSettled: () => {
    ...
    onSuccess: (data: unknown, { newStatus }) => {
      if (newStatus === "done" && data?.kpi_points) {
        message.success(
          `Hoàn thành xuất sắc! +${data.kpi_points} KPI points vào Ví thi đua.`,
          3
        );
      }
    },
