...
import { CheckCircleOutlined, SaveOutlined, PrinterOutlined } from '@ant-design/icons';
...
export const LabWorkspace = ({ request, onComplete }: Props) => {
    ...
    const evaluateResult = (val: string, conf: any) => {
        ...
        if (conf.value_type === 'quantitative') {
            ...
            if (conf.min_normal !== null && num < conf.min_normal) return <Tag color="red" className="m-0">↓ Thấp</Tag>;
            if (conf.max_normal !== null && num > conf.max_normal) return <Tag color="red" className="m-0">↑ Cao</Tag>;
            return <Tag color="green" className="m-0">Bình thường</Tag>;
        } else {
            // Định tính
            if (val === 'Dương tính' || val === 'Dương Tính' || val === '+') return <Tag color="red" className="m-0">Dương tính</Tag>;
            if (val === 'Âm tính' || val === 'Âm Tính' || val === '-') return <Tag color="green" className="m-0">Âm tính</Tag>;
            return <Tag color="default" className="m-0">Bình thường</Tag>;
        }
    };
    ...
    const handleSubmit = async (status: 'draft' | 'completed') => {
        ...
        if (absurdWarning && status === 'completed') {
            Modal.confirm({
                ...
                content: <div className="text-red-500 font-bold">{absurdWarning}. Bạn có chắc chắn muốn trả kết quả này?</div>,
    ...
    const columns = [
        ...
        },
        {
            ...
            render: (_: any, record: any, index: number) => {
                 ...
                 if (ev === 'High' || ev === 'Low' || ev === 'Positive') colorClass = "text-red-500 font-bold bg-red-50 focus:bg-red-100";
                 if (ev === 'Negative') colorClass = "text-green-600 font-bold bg-green-50 focus:bg-green-100";
                 ...
                 return (
                     <Input 
                         ...
                         onChange={(e) => handleValueChange(record.indicator_code, e.target.value)}
        ...
        },
        {
            ...
            align: 'center' as const,
            width: 120,
            render: (_: any, record: any) => evaluateResult(results[record.indicator_code], record)
        },
        {
            title: 'CSBT',
            dataIndex: 'range',
            ...
            render: (_: any, r: any) => {
                if (r.value_type === 'quantitative') {
                    if (r.min_normal && r.max_normal) return `${r.min_normal} - ${r.max_normal}`;
        ...
        <div className="flex flex-col h-full bg-white h-screen">
             <div className="p-4 border-b border-gray-100 flex justify-between items-center bg-white sticky top-0 z-10">
                  <div>
                      ...
                      <Text type="secondary">BN: {request.patient?.name || 'Khách vãng lai'} - Trạng thái: {request.status}</Text>
                  </div>
                  <div className="flex gap-3">
                      <Button icon={<PrinterOutlined />} onClick={handlePrint}>In Phiếu</Button>
                      <Button icon={<SaveOutlined />} onClick={() => handleSubmit('draft')} loading={isSubmitting}>Lưu Nháp</Button>
                      <Button type="primary" icon={<CheckCircleOutlined />} onClick={() => handleSubmit('completed')} loading={isSubmitting} style={{ backgroundColor: '#1890ff' }}>Hoàn Thành</Button>
                  </div>
             </div>
             ...
             <div className="flex-1 overflow-auto p-4 custom-scrollbar">
                  ...
                  <div className="mt-4 p-4 text-xs text-gray-400 bg-gray-50 rounded italic border border-gray-100">
                      * Mẹo Numpad UX: Nhập số rồi ấn `Enter` để nhảy xuống dòng kế tiếp. Nhập dấu `+` để ra Dương tĩnh, dấu `-` để ra Âm tính (đối với Định tính). Sử dụng phím `Lên/Xuống` tự do.
                  </div>
             </div>
        </div>
    );
