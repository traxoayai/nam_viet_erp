...
Deno.test(
  "loadGmailAccounts: single legacy account via GMAIL_USER_EMAIL + GMAIL_REFRESH_TOKEN",
  () => {
    const env = {
...
Deno.test(
  "loadGmailAccounts: stops at first gap (e.g., _2 missing skips _3)",
  () => {
    const env = {
      GMAIL_ACCOUNT_1_EMAIL: "a@gmail.com",
      GMAIL_ACCOUNT_1_REFRESH_TOKEN: "tok-a",
...
Deno.test(
  "loadGmailAccounts: partial account (email without token) is skipped",
  () => {
    const env = {
      GMAIL_ACCOUNT_1_EMAIL: "a@gmail.com",
