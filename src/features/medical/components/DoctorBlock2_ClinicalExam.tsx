...
export const DoctorBlock2_ClinicalExam: React.FC<Props> = ({ vitals, setVitals, clinical, setClinical, patient, readOnly, isVaccinationFlow = false }) => {
  ...
  const renderSpecializedForm = () => {
      if (age < 2) return <ExamForm_Infant data={clinical} onChange={handleClinicalChange} vitals={vitals} patientDOB={patient?.dob} />;
        ...
        <Card title="Khám Lâm Sàng" size="small" className="md:col-span-1 shadow-sm h-full">
            <div className="flex flex-col gap-3 h-full">
                <div>
                    ...
                    <Input.TextArea 
                        rows={2} placeholder="VD: Ho, sốt 3 ngày nay..." 
                        value={clinical.symptoms} 
                        onChange={e => handleClinicalChange('symptoms', e.target.value)}
                        className="mb-2" disabled={readOnly}
                    />
                    <div className={readOnly ? 'pointer-events-none opacity-60' : ''}>
                        <SmartScreeningChecklist 
                            age={age} clinical={clinical} 
                            onChange={handleClinicalChange} isVaccinationFlow={isVaccinationFlow}
                        />
                    </div>
                </div>
                ...
                <div className="flex-1">
                    ...
                    <Input.TextArea 
                        className="h-full" style={{ minHeight: '100px' }}
                ...
                <div>
                    ...
                    <Input 
                        className="font-bold" value={clinical.diagnosis} 
