...
export const SellPackageDrawer: React.FC<SellPackageDrawerProps> = ({
  open,
  onClose,
  customerId,
}) => {
    ...
    if (cartItems.length === 0)
      return (
        <div className="flex-1 bg-white rounded-xl border border-gray-200 shadow-sm p-4 flex flex-col items-center justify-center text-gray-400">
          <ShoppingCart size={48} className="text-gray-200 mb-2" /> Chưa có Gói
          nào được chọn
        </div>
      );
    return (
      <div className="flex-1 overflow-y-auto custom-scrollbar flex flex-col gap-3 pr-2">
        {cartItems.map((item) => (
          <div
            key={item.uid}
            className="bg-white rounded-lg border border-orange-200 shadow-sm p-3 relative"
          >
            <button
                  ...
                  <div className="flex flex-col gap-2 pl-2 border-l-2 border-orange-200">
                    {item.service_package_items.map(
                      (pi: unknown, i: number) => (
                        <div
                          key={i}
                          className="text-sm flex justify-between items-center py-1 border-b border-dashed border-gray-100 last:border-0"
                        >
    ...
    <Drawer
      title={
        <div className="flex items-center gap-2">
          <AppstoreAddOutlined className="text-white bg-[#fa8c16] p-1 rounded-full text-lg" />{" "}
          <span className="font-bold">Bán Gói Dịch Vụ (Ví)</span>
        </div>
      }
      placement="right"
      width={900}
      ...
      footer={
        <div className="flex justify-between items-center px-4 py-2">
          ...
          <div className="flex gap-2">
            ...
            <Button
              type="primary"
              style={{ backgroundColor: "#fa8c16", fontWeight: "bold" }}
      ...
      <div className="flex h-full">
        ...
        <div className="w-[40%] bg-white border-r border-gray-100 p-6 flex flex-col gap-6 h-full overflow-y-auto">
          <div>
            <div className="flex items-center gap-2 mb-3 text-gray-700 font-semibold border-b pb-2">
              <User size={18} className="text-orange-500" /> Thông Tin Khách
              Hàng
            </div>
          ...
          <div className="flex-1">
            <div className="flex items-center gap-2 mb-3 text-gray-700 font-semibold border-b pb-2">
              <Plus size={18} className="text-green-500" /> Chọn Gói Cần Bán
            </div>
            <Select
              className="w-full"
              showSearch
              placeholder="Gõ tên Gói..."
        ...
        <div className="w-[60%] bg-slate-50 p-6 flex flex-col h-full">
          <div className="flex items-center gap-2 mb-4 text-gray-700 font-bold text-lg">
            <ShoppingCart size={20} className="text-[#fa8c16]" /> Giỏ Hàng Gói
            Dịch Vụ
          </div>
          {renderCartItems()}
        </div>
