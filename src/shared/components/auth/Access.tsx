...
export const Access: React.FC<AccessProps> = ({ 
  permission, children, fallback = null, hide = false 
}) => {
  ...
  if (React.isValidElement(children)) {
     return (
        <Tooltip title="Bạn không có quyền thực hiện thao tác này">
            <span style={{ cursor: 'not-allowed', opacity: 0.6, display: 'inline-block' }}>
                {React.cloneElement(children as React.ReactElement, { disabled: true, onClick: undefined } as any)}
            </span>
        </Tooltip>
     );
  }
