...
const UniversalCustomerSelect: React.FC<Props> = ({ 
  ...
  const fetchCustomers = useMemo(() => debounce(async (search: string) => {
    if (!search.trim()) {
        setOptions([]);
        return;
    }
    ...
    try {
        ...
        const b2bOptions = b2bData.map((c: any): CustomerOption => ({
            ...
            label: (
                <div style={{display:'flex', justifyContent:'space-between', alignItems:'center'}}>
                    <div style={{display:'flex', gap: 8, alignItems:'center'}}>
                        <Avatar size="small" icon={<ShopOutlined />} style={{backgroundColor: '#eb2f96'}} />
                        <div>
                            <Text strong>{c.name}</Text>
                            <div style={{fontSize: 10, color:'#888'}}>{c.phone} | MST: {c.tax_code}</div>
                        </div>
                    </div>
        ...
        // Map B2C (Khách lẻ)
        const b2cOptions = b2cData.map((c: any): CustomerOption => ({
            key: `B2C_${c.id}`,
            value: c.id,
            type: 'B2C',
            rawName: c.name,
