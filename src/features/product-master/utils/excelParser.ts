...
export const generateExcelTemplate = (
  ...
  const flattenedData = data.map((item) => {
    ...
    warehouses.forEach((wh) => {
      const settings = item.warehouse_settings?.find(
        (w) => w.warehouse_id === wh.id
      );
  ...
  const colWidths = Object.keys(flattenedData[0] || {}).map(() => ({
    wch: 20,
  }));
  ws["!cols"] = colWidths;
...
export const parseExcelToPayload = (
  ...
  return data
    .map((row: any) => {
      ...
      if (row["Tên sản phẩm"] !== undefined)
        payload.name = getVal("Tên sản phẩm") as string | undefined;
      if (row["Trạng thái"] !== undefined)
        payload.status = getVal("Trạng thái") as string | undefined;
      if (row["Ảnh (URL)"] !== undefined)
        payload.image_url = getVal("Ảnh (URL)") as string | undefined;
      if (row["Mã vạch"] !== undefined)
        payload.barcode = getVal("Mã vạch") as string | undefined;
      if (row["Nhà sản xuất"] !== undefined)
        payload.manufacturer_name = getVal("Nhà sản xuất") as
          | string
          | undefined;
      if (row["Giá vốn (Cost)"] !== undefined)
        payload.cost_price = getVal("Giá vốn (Cost)", "number") as
          | number
          | undefined;

      if (row["Đơn vị Cơ bản"] !== undefined)
        payload.base_unit_name = getVal("Đơn vị Cơ bản") as string | undefined;

      if (row["Đơn vị Lẻ"] !== undefined)
        payload.retail_unit_name = getVal("Đơn vị Lẻ") as string | undefined;
      if (row["Quy đổi Lẻ"] !== undefined)
        payload.retail_conversion_rate = getVal("Quy đổi Lẻ", "number") as
          | number
          | undefined;
      ...
      if (row["Đơn vị Sỉ"] !== undefined)
        payload.wholesale_unit_name = getVal("Đơn vị Sỉ") as string | undefined;
      if (row["Quy đổi Sỉ"] !== undefined)
        payload.wholesale_conversion_rate = getVal("Quy đổi Sỉ", "number") as
          | number
          | undefined;
      ...
      if (warehouseMap.size > 0) {
        payload.warehouse_settings = Array.from(warehouseMap.entries()).map(
          ([id, settings]) => ({
            warehouse_id: id,
    ...
    .filter(
      (p): p is ProductMasterImportPayload => p !== null && p !== undefined
    );
};
